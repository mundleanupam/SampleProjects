...
public class Autocomplete {
    ...
    // Trie node class
    private class AutoNode {
        String prefix;
        HashMap<Character, AutoNode> children;
        boolean isWord;
        // Does this node represent the last character in a word?

        private AutoNode(String prefix) {
            this.prefix = prefix;
            this.children = new HashMap<Character, AutoNode>();
        }
    }

    // The trie
    private AutoNode trie;
    ...
    private void insertWord(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            if (!curr.children.containsKey(s.charAt(i))) {
                curr.children.put(s.charAt(i), new AutoNode(s.substring(0, i + 1)));
            }
    ...
    private void findAllChildWords(AutoNode n, List<String> results) {
